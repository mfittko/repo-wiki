...
test('compileWiki in LLM mode synthesizes module pages through the mock provider', async () => {
  ...
  try {
    ...
    assert.match(modulePage, /kind: "module"/);
    assert.match(modulePage, /source_commit: "mock"/);
    assert.match(modulePage, /# Auth/);
    assert.match(modulePage, /Generated by the mock LLM provider/);

    // No LLM errors
    assert.ok(!result.summary.llm_errors || result.summary.llm_errors.length === 0);
    // Module page counted
...
test('compileWiki in LLM mode does not overwrite existing page when provider output is invalid', async () => {
  ...
  try {
    const result = await compileWiki({ scanDir, planFile, wikiDir, config, _provider: badProvider });
    ...
    // Actionable error returned in summary.
    assert.ok(Array.isArray(result.summary.llm_errors));
    assert.equal(result.summary.llm_errors.length, 1);
    assert.equal(result.summary.llm_errors[0].file, 'Module-Auth.md');
    assert.ok(result.summary.llm_errors[0].error.length > 0);
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
  }
});
...
test('compileWiki deterministic mode is unaffected by config presence', async () => {
  ...
  try {
    ...
    assert.ok(!result.summary.llm_errors);
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
  }
});
